...
lazy val aggregatorIDs = Seq("xml")
...
lazy val sharedSettings = Seq(
  projectTitle := "Vesper Codecs",
  projectWebsiteRootURL := "https://ing-vesper.github.io/",
  projectWebsiteBasePath := "/vesper-codecs/",
  githubOwnerID := "ing-vesper",
  githubRelativeRepositoryID := "vesper-codecs",

  organization := "io.github.ing-vesper",
  scalaVersion := "2.13.3",
...
lazy val root = project.in(file("."))
  ...
  .aggregate(xmlJVM, xmlJS)
  .configure(defaultPlugins)
  .settings(sharedSettings)
  .settings(doNotPublishArtifact)
  .settings(unidocSettings(xmlJVM))
  .settings(
...
lazy val site = project.in(file("site"))
  ...
  .settings {
    ...
    Seq(
      ...
      micrositeDescription := "Scala library for encoding/decoding XML documents.",
